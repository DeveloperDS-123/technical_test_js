...
export const generateRecommendations = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const apiResponse = await axios.post(' http://host.docker.internal:8080/llm/generate', { preferences });
    const { recommendations } = apiResponse.data;

    // Save to the database
    const newRecommendation = new RecommendationModel({ user_id, recommendations });
